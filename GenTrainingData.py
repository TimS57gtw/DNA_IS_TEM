...
def gen_DNA_shape_OLD():

    width = np.random.uniform(RESOLUTION / 6, RESOLUTION/3)
    length = np.random.uniform(RESOLUTION / 6, RESOLUTION/3)


    vertices = []
    border_vertices = []
    left_x = -width / 2
    right_x = width - width / 2
    lower_y = -length / 2
    upper_y = length - length / 2
    no_vertices_x = 10
    no_vertices_y = 10
    height_var = abs(np.random.normal(0.1, 0.1))/2
    perlin_x_var = abs(np.random.normal(0.1, 0.1))
    perlin_y_var = abs(np.random.normal(0.1, 0.1))
    # height via perlin noise
    #get_vert_z = lambda : np.random.normal(1, 0.1)
    # print("LX: ", left_x.px, " LY ", lower_y.px)
    height_noise = PerlinNoise(octaves=no_vertices_y/10)
    h_vals = [[height_noise([i / no_vertices_x, j / no_vertices_y]) for j in range(no_vertices_x)] for i in range(no_vertices_y)]
    h_vals /= (np.std(h_vals) / height_var)
    h_vals += 1
    get_vert_z = lambda i, j: h_vals[i, j]
    mrgrid_dx = (right_x - left_x) / (no_vertices_x - 1)
    mrgrid_dy = (upper_y - lower_y) / (no_vertices_y - 1)
    perlin_x = PerlinNoise(octaves=no_vertices_x/10)
    x_shift_vals = [[perlin_x([i / no_vertices_x, j / no_vertices_y]) for j in range(no_vertices_x)] for i in range(no_vertices_y)]
    x_shift_vals /= (np.std(x_shift_vals) / perlin_x_var)
    get_shift_x = lambda i, j: x_shift_vals[i, j] * mrgrid_dx
    perlin_y = PerlinNoise(octaves=no_vertices_y/10)
    y_shift_vals = [[perlin_y([i / no_vertices_x, j / no_vertices_y]) for j in range(no_vertices_x)] for i in range(no_vertices_y)]
    y_shift_vals /= (np.std(y_shift_vals) / perlin_y_var)
    get_shift_y = lambda i, j: y_shift_vals[i, j] * mrgrid_dy
    rect_grid = np.zeros((no_vertices_x-1, no_vertices_y-1), dtype=object)
    vrtx_grid = np.zeros((no_vertices_x, no_vertices_y), dtype=object)
    for i in range(no_vertices_x):
        for j in range(no_vertices_y):
            vertex = np.array([left_x + i * mrgrid_dx + get_shift_x(i, j), lower_y + j * mrgrid_dy + get_shift_y(i, j), get_vert_z(i, j)])
            vrtx_grid[i, j] = copy.deepcopy(vertex)
            if i == 0 or j == 0 or i == no_vertices_x - 1 or j  == no_vertices_y - 1:
                border_vertices.append(copy.deepcopy(vertex))
    for i in range(1, no_vertices_x):
        for j in range(1, no_vertices_y):
            rect_grid[i-1, j-1] = Rect( vrtx_grid[i-1, j-1], vrtx_grid[i, j-1],vrtx_grid[i-1, j], vrtx_grid[i, j] )
            # print(rect_grid[i-1, j-1])
    mod_rect_grid = rect_grid

    def access_mod_rect_grid(x, y):
        left_x = -width / 2
        right_x = width - width / 2
        lower_y = -length / 2
        upper_y = length - length / 2
        loc_x = x - left_x
        loc_y = y - lower_y
        grid_idx_i = int(np.floor(loc_x / mrgrid_dx))
        grid_idx_j = int(np.floor(loc_y / mrgrid_dx))
        rect = None
        try:
            rect = mod_rect_grid[grid_idx_i, grid_idx_j]
            if not rect.contains([x, y]):
                rect = None
        except IndexError:
            pass
        if rect is None:
            for candi in mod_rect_grid.flatten():
                if candi.contains([x, y]):
                    rect = candi
                    break
        if rect is None:
            return 0
        return rect.elevation([x, y])

    arr = np.zeros((RESOLUTION, RESOLUTION))



    centerx = RESOLUTION / 2 + np.random.uniform(-RESOLUTION/4, RESOLUTION/4)
    centery = RESOLUTION / 2 + np.random.uniform(-RESOLUTION/4, RESOLUTION/4)
    ...
    for i in tqdm(range(RESOLUTION)):
        for j in range(RESOLUTION):
            ...
            arr[j, i] = access_mod_rect_grid(x, y)
...
#
   ...
   #  plt.show()
    ...
    ks = 41
    mid = 21
    kernelsig = np.random.uniform(5, 30)
...
def generate_image(save_fn, label_fn):
    arr = np.zeros((RESOLUTION, RESOLUTION))
    ...
    noise_sig = 0.05 + 0.2 * np.random.random()
    ...
    diam_x = np.random.normal(100, 20, no_spots)
    diam_y = np.random.normal(100, 20, no_spots)
    heights = []
...
class gen_parallel(Process):
    def __init__(self, fnames, resf, lblf, tqd):
        super().__init__()
...
#
   #  while False:
    ...
    outfld = os.path.join('SynthData', 'Set4', 'image')
    outlbl = os.path.join('SynthData', 'Set4', 'label')
    ...
    thrds = []
    for i in range(threads):
        thrds.append(gen_parallel(fn_lists[i], outfld, outlbl, i==0))
